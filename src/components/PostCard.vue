...
<template>
  <div
    ...
    <UserInfo
      ...
      :avatar-url="avatar"
      :title="name"
      :subtitle="toLocaleDate(createdAt)"></UserInfo>
      ...
      <div class="dark:dark-card custom-border flex flex-1 items-center">
        ...
        <div
          ...
          <IconLoad
            ...
            class="ml-2 h-4 w-4 animate-spin"></IconLoad>
        </div>
      </div>
    </div>

    <div
      v-for="cmt of innerComments"
      :key="cmt._id"
      class="mt-4 rounded-lg bg-bg-light p-4 dark:border-gray-500 dark:bg-gray-800">
      <UserInfo
        :size="40"
        :img-url="cmt.user.avatar"
        :title="cmt.user.name"
        :subtitle="toLocaleDate(cmt.createdAt)"></UserInfo>
